...
class Array(object):
    ...
    def insert_at(self, position, item):
        if position < 0 or position > self.__nItems:  # Check position validity
            raise IndexError("Position out of bounds")
        if self.__nItems >= len(self.__a):  # Check if there is space in the array
            raise IndexError("Array is full")
        for j in range(self.__nItems, position, -1):  # Switch elements to the right
            self.__a[j] = self.__a[j - 1]
        self.__a[position] = item  # Insert the item
        self.__nItems += 1  # Increment the count of item
