...
class LinkedList:
    ...
    # call function fun for each data in the list
    def traverse(self, fun=print):
        link = self.get_first()
    ...
    def __str__(self):
        result = "["
        link = self.get_first()
        while link is not None:     # using __iter__
            if len(result) > 1:
                result += " > "
            result += str(link.get_data())
            link = link.get_next()
        return result + "]"
    ...
    # compared to book listing
    def delete(self, goal, key=identity):
        if self.is_empty():
            raise RuntimeError("Cannot delete from empty list")
        # previous is link before goal link
        # At first not a Link but LinkedList (self), works because we 
        # implemented get_next and set_next for LinkedList
        previous = self
